...
export async function generateMetadata({ params }: { params: Promise<{slug:string}> }): Promise<Metadata> {
  ...
  return { ...meta, alternates:{ canonical:`https://cmsv6.co.uk/${slug}` }, openGraph:{title:meta.title,description:meta.description,url:`https://cmsv6.co.uk/${slug}`} };
}
...
export default async function StandardPage({params}:{params:Promise<{slug:string}>}) {
  ...
  return <main><PageHero eyebrow={hero[0]} title={hero[1]} text={hero[2]}>{slug === "pricing" && <a className="button primary" href="#plans">See hosting plans</a>}{slug === "contact" && <a className="button primary" href="#enquiry">Start an enquiry</a>}</PageHero>{renderPage(slug)}{!["contact","privacy","terms","responsible-use"].includes(slug) && <CTA/>}</main>;
}
...
function renderPage(slug:string) {
  switch(slug) {
    case "features": return <><section className="section"><div className="container"><SectionHeading eyebrow="Core capabilities" title="Live, locate, review and manage." text="Each feature is available where supported by the connected equipment and configuration."/><div className="feature-grid detailed">{features.map((f,i)=><article className="feature-card" key={f.title}><span className="feature-icon">{f.icon}</span><small>0{i+1}</small><h3>{f.title}</h3><p>{f.text}</p></article>)}</div><Disclaimer/></div></section><section className="section pale"><div className="container two-column"><div><h2>Local recording. Remote connectivity.</h2><p>A compatible DVR records footage to its installed local storage. CMSV6 provides remote access while the DVR has a suitable connection. If mobile data or Wi-Fi drops, local recording does not necessarily stop; live viewing and remote requests resume when connectivity returns, depending on the setup.</p></div><div className="info-card"><h3>Flexible access</h3><ul className="tick-list dark-ticks"><li>Supported web browser</li><li>Windows desktop client</li><li>Mobile application</li><li>Phone, tablet, laptop or desktop</li></ul><p className="small">Approved download links will be added only after exact application and server compatibility is confirmed.</p></div></div></section></>;
    case "solutions": return <><section className="section"><div className="container"><SectionHeading eyebrow="Across every environment" title="One monitoring platform, many practical uses." text="Choose the view, connectivity and compatible equipment around the environment — not the other way around."/><div className="solution-list">{industries.map((item,i)=><article key={item.title}><div className="solution-image" style={{backgroundImage:`url(${item.image})`}}/><div><span className="eyebrow">0{i+1} · {item.label}</span><h2>{item.title}</h2><p>{item.text}</p><ul>{solutionBullets[i].map(x=><li key={x}>{x}</li>)}</ul><a className="text-link" href="/contact">Discuss this solution →</a></div></article>)}</div></div></section><ResponsibleSector/></>;
...
function SupportPage(){ const topics=["New customer setup","Device activation","Login and password help","Adding a device","Mobile-data and Wi-Fi setup","GPS troubleshooting","Camera troubleshooting","Playback and video download","Device offline checks","App and desktop-client guidance","Hosting renewal","Manuals and downloads"]; return <><section className="section"><div className="container"><SectionHeading eyebrow="Support centre" title="Choose the area you need help with." text="Never share server credentials, administrator passwords or customer device IDs in a public message."/><div className="support-grid">{topics.map((x,i)=><a href="#troubleshooting" key={x}><span>{String(i+1).padStart(2,"0")}</span><h3>{x}</h3><b>Get guidance →</b></a>)}</div></div></section><section className="section pale" id="troubleshooting"><div className="container two-column"><div><SectionHeading eyebrow="First checks" title="Device showing offline?" text="Work through these steps before changing settings."/><div className="faq-list compact-faq">{["Confirm the DVR still has power and is recording locally.","Check that the supported internet connection is active.","Check SIM data allowance, signal or Wi-Fi/network status.","Confirm antennas and approved cables are firmly connected.","Note any warning shown and the last time the device reported."].map((x,i)=><details key={x}><summary>{i+1}. {x}</summary><p>Record what you find and contact support before resetting or changing advanced settings. A connection loss does not necessarily stop normal local DVR recording.</p></details>)}</div></div><div className="support-contact" id="login"><span className="eyebrow light-blue">Existing customer?</span><h2>Account and login assistance</h2><p>The approved CMSV6 login address will be published here once exact server and application compatibility is confirmed. We will not guess or expose private access details.</p><a className="button light" href="/contact?enquiry=support">Contact support</a><a className="text-link light-link" href="tel:+447543025555">Call +44 (0) 7543 02 5555 →</a></div></div></section></> }
