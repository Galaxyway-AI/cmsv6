...
export const metadata: Metadata = {
  title: "CMSV6 UK | Live CCTV Video, GPS Tracking & Remote DVR Access",
  description: "UK-hosted CMSV6 live CCTV, remote DVR playback and GPS monitoring for vehicles, businesses, farms, boats and fleets. View plans and compatible iCustodian DVRs.",
  alternates: { canonical: "https://cmsv6.co.uk/" },
};

const industryLinks=["/solutions/vehicle-cctv","/solutions/business-cctv","/solutions/farm-cctv","/solutions/boat-marine-cctv"];
const featureLinks=["/resources/live-cctv-versus-recorded-playback","/resources/cmsv6-gps-tracking-guide","/resources/live-cctv-versus-recorded-playback","/features","/solutions/fleet-cctv-and-video-telematics","/features"];
const homeServiceSchema={"@context":"https://schema.org","@type":"Service","@id":"https://cmsv6.co.uk/#service",name:"iCustodian CMSV6 hosting",description:"UK-hosted remote CCTV, live-video, audio, playback and GPS monitoring for compatible equipment.",url:"https://cmsv6.co.uk/",provider:{"@id":"https://cmsv6.co.uk/#organisation"},areaServed:{"@type":"Place",name:"United Kingdom and international locations subject to availability"},offers:[{"@type":"Offer",name:"Six months CMSV6 hosting",price:"60",priceCurrency:"GBP",url:"https://cmsv6.co.uk/pricing"},{"@type":"Offer",name:"Twelve months CMSV6 hosting",price:"100",priceCurrency:"GBP",url:"https://cmsv6.co.uk/pricing"},{"@type":"Offer",name:"Twenty-four months CMSV6 hosting",price:"180",priceCurrency:"GBP",url:"https://cmsv6.co.uk/pricing"}]};
...
export default function Home() {
  return <main>
    ...
    <section className="section free-strip" id="free-offer"><div className="container free-layout"><div className="free-number"><span>2</span><small>months<br/>free</small></div><div><span className="eyebrow red">Included with compatible equipment</span><h2>Buy a compatible iCustodian® DVR and start with two months of CMSV6 hosting free.</h2><p>iCustodian® CMSV6 is a UK-hosted remote CCTV, live-video, audio and GPS monitoring platform for compatible vehicles, premises, farms, boats and remote locations. The free period lets qualifying buyers configure and test supported remote functions before choosing a paid term.</p></div><div className="stack-actions"><a className="button primary" href="https://www.icustodian.com/product-category/dvr/?utm_source=cmsv6.co.uk&utm_medium=referral&utm_campaign=cmsv6_equipment&utm_content=homepage_free_offer" rel="noreferrer" data-analytics-event="icustodian_category_clicked">View compatible DVRs</a><a className="text-link" href="/cmsv6-device-compatibility">Check my device →</a></div></div></section>

    <section className="section"><div className="container"><SectionHeading eyebrow="The platform" title="See it live. Know where it is. Review what happened." text="CMSV6 brings the most useful remote-monitoring tools together in a clear system, with availability shaped by your connected equipment and configuration."/><div className="feature-grid">{features.map((item,index) => <article className="feature-card" key={item.title}><span className="feature-icon">{item.icon}</span><h3>{item.title}</h3><p>{item.text}</p><a href={featureLinks[index]}>Explore {item.title.toLowerCase()} →</a></article>)}</div><Disclaimer/></div></section>

    <section className="section pale"><div className="container"><SectionHeading eyebrow="One platform. Limitless applications." title="Designed for the road — and everywhere beyond it." text="The same remote technology can support visibility across moving assets and fixed locations. CMSV6 is not simply a vehicle tracker."/><div className="industry-grid">{industries.map((industry, index) => <article className="industry-card" key={industry.title} style={{backgroundImage:`linear-gradient(180deg, transparent 20%, rgba(4,15,31,.92) 100%), url(${industry.image})`}}><span>0{index+1} · {industry.label}</span><div><h3>{industry.title}</h3><p>{industry.text}</p><a href={industryLinks[index]}>Explore {industry.title.toLowerCase()} →</a></div></article>)}</div></div></section>
